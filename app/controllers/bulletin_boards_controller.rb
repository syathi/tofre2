class BulletinBoardsController < ApplicationController
    ...
    def index
        ...
        @public_foods = Refrigerator.where.not(deadline: nil).where(release: 1).order(:change)
        @public_fresh= Refrigerator.where(deadline: nil).where(release: 1).order(:change)
